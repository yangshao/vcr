...
class SimpleDetector(nn.Module):
    def __init__(self, pretrained=True, average_pool=True, semantic=True, final_dim=1024):
        ...
        if average_pool:
            ...
            after_roi_align += [nn.AvgPool2d(14, stride=1), Flattener()]
    ...
    def forward(self,
        ...
        roi_align_res = self.roi_align(img_feats.float(), rois)
        if config.double_flag:
            roi_align_res = roi_align_res.double()
        if self.mask_upsample is not None:
            assert segms is not None
            segms_indexed = segms[box_inds[:, 0], None, box_inds[:, 1]] - 0.5
            if config.double_flag:
                segms_indexed = segms_indexed.double()
            roi_align_res[:, :self.mask_dims] += self.mask_upsample(segms_indexed)

        # actual_ct, dim, cur_w, cur_h = roi_align_res.size()
        # new_obj_reps = pad_sequence(roi_align_res, box_mask.sum(1).tolist())
        # bs, max_ct, _, _, _ = new_obj_reps.size()
        # #take the mean over 14*14 for specific objects
        # new_obj_reps_mean = torch.mean(new_obj_reps.view(bs, max_ct, dim, -1), -1, True).squeeze()
        ...
        return {
            ...
            # 'new_2d_objs': new_obj_reps,
            # 'new_1d_obj_reps': new_obj_reps_mean,
            # 'ori_images': images
        }
