...
from dataloaders.beam_vcr_test import  VCR, VCRLoader
...
val, test = VCR.splits(mode='rationale' if args.rationale else 'answer',
...
loader_params = {'batch_size': 50 // NUM_GPUS, 'num_gpus':NUM_GPUS, 'num_workers':num_workers}
...
model = Model.from_params(vocab=Vocabulary(), params=params['model'])
print('*'*100)
...
# for eval_set, name in [(val_loader, 'val')]:
    ...
    np.save(os.path.join(args.folder, f'beam{name}preds.npy'), test_probs)