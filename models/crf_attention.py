...
for eval_set, name in [(test_loader, 'test'), (val_loader, 'val')]:
    ...
    for b, (time_per_batch, batch) in enumerate(time_batch(eval_set)):
        with torch.no_grad():
            batch = _to_gpu(batch)
            output_dict = model.get_att(**batch)
            qa_atts.append(output_dict['qa_att'].detach().cpu().numpy())
            qr_atts.append(output_dict['qr_att'].detach().cpu().numpy())
    qa_atts = np.concatenate(qa_atts, 0)
    qr_atts = np.concatenate(qr_atts, 0)
    np.save(os.path.join(args.folder, f'{name}_qa_att.npy'), qa_atts)
    np.save(os.path.join(args.folder, f'{name}_qr_att.npy'), qr_atts)
