...
def fix_kb(kb_sent, cur_item, ori_item):
    cur_objs = cur_item['objects']
    ori_objs = ori_item['objects']
    #first feed in the non-people from the original item's objects
    people_dic = defaultdict()
    other_dic =  defaultdict()
    person_list = ['person', 'people']
    name_idx = 0
    for tok in kb_sent:
        if isinstance(tok, list):
            for int_name in tok:
                obj_type = ori_objs[int_name]
                if obj_type not in person_list and int_name not in other_dic:
                    other_dic[int_name] = obj_type
                elif obj_type in person_list:
                    if int_name not in people_dic:
                        people_dic[int_name] = name_idx
                        name_idx += 1
    res = []
    for tok in kb_sent:
        if isinstance(tok, list):
            for int_name in tok:
                if(int_name in other_dic):
                    name = other_dic[int_name]
                elif int_name in people_dic:
                    name_idx = people_dic[int_name]
                    name = GENDER_NEUTRAL_NAMES[name_idx]
                res.append(name)
        else:
            res.append(tok)
    return res
...
def get_mean_field(field):
    ...
    tokens = field[0].tokens
    mean_field = BertField([token for token in tokens],
                           mean_array,
                           padding_value=0)
    return mean_field
...
class VCR(Dataset):
    def __init__(self, split, only_use_relevant_dets=True, add_image_as_a_box=False, embs_to_load='bert_da', aug_flag=True):
        ...
        self.split = split
        self.rationale_topk = 10
        if split == 'train':
            with open(os.path.join(config.VCR_ANNOTS_DIR, 'aug_{}.jsonl'.format(split)), 'r') as f:
                self.items = [json.loads(s) for s in f]
        else:
            with open(os.path.join(config.VCR_ANNOTS_DIR, 'aug_{}_kb.jsonl'.format(split)), 'r') as f,\
                    open(os.path.join(config.VCR_ANNOTS_DIR, 'aug_train.jsonl'), 'r') as train_f:
                self.train_items = [json.loads(s) for s in train_f]
                self.train_h5fn = os.path.join(config.VCR_ANNOTS_DIR, f'aug_bert_rationale_train.h5')
                self.items = [json.loads(s) for s in f]
    ...
    def _get_dets_to_use(self, item, rationale_flag = True):
        """
        ...
        if rationale_flag:
            ...
            all_rationale_choices = item['aug_rationales']
            answer_related_rationale_idx = item['answer_label']
            start_idx = answer_related_rationale_idx*4
            end_idx = (answer_related_rationale_idx+1)*4
            rationale_choices = [all_rationale_choices[i] for i in range(start_idx, end_idx)]
        else:
            rationale_choices = []
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            #use only the original rationales
            all_rationale_choices = item['aug_rationales']
            answer_related_rationale_idx = item['answer_label']
            start_idx = answer_related_rationale_idx*4
            end_idx = (answer_related_rationale_idx+1)*4
            rationale_choices = [(i, all_rationale_choices[i]) for i in range(start_idx, end_idx)]
            dets2use, old_det_to_new_ind = self._get_dets_to_use(item)
        else:
            kb_embeddings =  []
            #collect topk rationales for each answer
            dumy_sent = ['test' for i in range(5)]
            dummy_emb = np.random.uniform(-0.01, 0.01, (5, 768))
            rationale_mask = []
            kbs = item['kbs']
            rationale_choices = []
            for i in range(4):
                ...
                n = len(cur_ans_kbs)
                cur_rationale_mask = []
                for j in range(self.rationale_topk):
                    if(j>=n):
                        rationale_choices.append(dumy_sent)
                        cur_rationale_mask.append(0)
                        kb_embeddings.append(dummy_emb)
                    else:
                        sample_idx, rationale_idx = cur_ans_kbs[j]
                        sample_idx = int(sample_idx)
                        rationale_idx = int(rationale_idx)
                        with h5py.File(self.train_h5fn, 'r') as train_h5:
                            kb_grp_items = {k: np.array(v, dtype=np.float16) for k,v in train_h5[str(sample_idx)].items()}
                        kb_rationale = self.train_items[sample_idx]['aug_rationales'][rationale_idx]
                        kb_rationale = fix_kb(kb_rationale, item, self.train_items[sample_idx])
                        rationale_choices.append(kb_rationale)
                        cur_rationale_mask.append(1)
                        key = 'answer_qr'+str(rationale_idx)
                        rationale_emb = kb_grp_items[key]
                        kb_embeddings.append(rationale_emb)
                rationale_mask.append(cur_rationale_mask)

            rationale_mask = np.array(rationale_mask)
            instance_dict['rationale_mask'] = ArrayField(rationale_mask)


            dets2use, old_det_to_new_ind = self._get_dets_to_use(item, rationale_flag=False)
        ...
        if self.split != 'train':
            mean_question_tokenized = get_mean_field(questions_tokenized)
            mean_question_tag = question_tags[0]
            mean_question_tag.sequence_field = mean_question_tokenized

        # r_ct = len(rationale_choices)
        r_ct = 16
        real_r_ct =  len(rationale_choices)
        ...
        if self.split == 'train':
            ...
            rationales_tokenized, rationale_tags = zip(*[_fix_tokenization(
                ...
                token_indexers=self.token_indexers,
                pad_ind=0 if self.add_image_as_a_box else -1
            ) for i, rationale in rationale_choices])
            ...
            rationale_label = item['aug_answer_rationale_indexes'][item['answer_label']]
            real_rationale_label = rationale_label - item['answer_label']*4
            # instance_dict['rationale_label'] = LabelField(item['aug_answer_rationale_indexes'][item['answer_label']], skip_indexing=True)
            instance_dict['rationale_label'] = LabelField(real_rationale_label, skip_indexing=True)
        else:
            instance_dict['qr_question'] = ListField([questions_tokenized[0] for i in range(real_r_ct)])
            instance_dict['qr_question_tags'] = ListField([question_tags[0] for i in range(real_r_ct)])
            rationales_tokenized, rationale_tags = zip(*[_fix_tokenization(
                rationale,
                kb_embeddings[i],
                old_det_to_new_ind,
        ...
        if self.split == 'train':
            ...
            rationales_tokenized, rationale_tags = zip(*[_fix_tokenization(
                ...
                token_indexers=self.token_indexers,
                pad_ind=0 if self.add_image_as_a_box else -1
            ) for j, answer in enumerate(answer_choices) for i, rationale in rationale_choices])
            ...
            answers_tokenized, answer_tags = zip(*[_fix_tokenization(
                answer,
                # grp_items[f'answer_{temp_key}{i}'],
                answers_tokenized[i].embs,
                old_det_to_new_ind,
                item['objects'],
                token_indexers=self.token_indexers,
                pad_ind=0 if self.add_image_as_a_box else -1
