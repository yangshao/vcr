...
class VCR(Dataset):
    def __init__(self, split, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da', aug_flag=True):
        self.split = split
        ...
        print('split: ', split)
        print('number of {} samples: {}'.format(self.split, len(self.items)))
    ...
    def __getitem__(self, index):
        ...
        instance_dict['a_question'] = ListField(questions_tokenized)
        instance_dict['a_question_tags'] = ListField(question_tags)
        ...
        instance_dict['r_question'] = ListField(questions_tokenized)
        instance_dict['r_question_tags'] = ListField(question_tags)
        ...
        instance_dict['rationales'] = ListField(rationales_tokenized)
        instance_dict['rationale_tags'] = ListField(rationale_tags)
...
def collate_fn(data, to_gpu=False):
    ...
    td['a_question_mask'] = get_text_field_mask(td['a_question'], num_wrapping_dims=1)
    td['a_question_tags'][td['a_question_mask'] == 0] = -2  # Padding
    td['r_question_mask'] = get_text_field_mask(td['r_question'], num_wrapping_dims=1)
    td['r_question_tags'][td['r_question_mask'] == 0] = -2  # Padding
