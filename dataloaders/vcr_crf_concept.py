...
class VCR(Dataset):
    def __init__(self, split, only_use_relevant_dets=True, add_image_as_a_box=False, embs_to_load='bert_da', aug_flag=True):
        ...
        with open(os.path.join(config.VCR_ANNOTS_DIR, 'aug_{}_concept.jsonl'.format(split)), 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        question = questions_tokenized[0].tokens
        answer_choices = [temp.tokens for i, temp in enumerate(answers_tokenized) if i%16==0]
        rationale_choices = [temp.tokens for i, temp in enumerate(rationales_tokenized) if i < 16]

        question_items = question
        question_l = []
        for token in question_items:
            token = token.text
            assert token in words2index, token
            question_l.append(int(words2index[token]))
        instance_dict['question_concept'] =  ListField([LabelField(x, skip_indexing=True) for x in question_l])

        answer_ls = []
        for i, answer in enumerate(answer_choices):
            answer_l = []
            answer_items = answer
            for token in answer_items:
                token = token.text
                assert token in words2index, token
                answer_l.append(int(words2index[token]))
            answer_l = ListField([LabelField(x, skip_indexing=True) for x in answer_l])
            answer_ls.append(answer_l)
        instance_dict['answer_concept'] = ListField(answer_ls)

        rationale_ls = []
        for i, rationale in enumerate(rationale_choices):
            rationale_l = []
            rationale_items = rationale
            for token in rationale_items:
                token = token.text
                assert token in words2index, token
                rationale_l.append(int(words2index[token]))
            rationale_l = ListField([LabelField(x, skip_indexing=True) for x in rationale_l])
            rationale_ls.append(rationale_l)
        instance_dict['rationale_concept'] = ListField(rationale_ls)



        # #load concept embedding
        # answer_embs = []
        # for answer_keys in item['answer_concepts']:
        #     temp = []
        #     for key in answer_keys:
        #         assert(key in self.concept_embedding)
        #         key_emb = self.concept_embedding[key]
        #         temp.append(key_emb)
        #
        #     if(len(temp)==0):
        #         answer_keys = ['dump_key']
        #         temp.append(np.ones(300))
        #     answer_concept_emb = np.stack(temp)
        #     answer_concept_field = BertField([Token(x) for x in answer_keys],
        #                     answer_concept_emb,
        #                    padding_value=0)
        #     answer_embs.append(answer_concept_field)
        # answer_embs = ListField(answer_embs)
        # instance_dict['answer_concepts'] = answer_embs
        #
        # rationale_embs = []
        # for rationale_keys in item['rationale_concepts']:
        #     temp = []
        ...
        #
        #     if(len(temp)==0):
        #         rationale_keys = ['dump_key']
        #         temp.append(np.ones(300))
        #     rationale_concept_emb = np.stack(temp)
        #     rationale_concept_field = BertField([Token(x) for x in rationale_keys],
        #                                      rationale_concept_emb,
        #                                      padding_value=0)
        #     rationale_embs.append(rationale_concept_field)
        # rationale_embs = ListField(rationale_embs)
        # instance_dict['rationale_concepts'] = rationale_embs
