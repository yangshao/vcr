...
def process(i, items):
    return items[i][0], np.array(items[i][1], dtype=np.float16)

class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da', aug_flag=True):
        self.split = split
        self.mode = mode
        ...
        if mode not in ('answer', 'rationale', 'crf'):
            raise ValueError("split must be answer or rationale or crf")
    ...
    def _get_dets_to_use(self, item):
        ...
        if(self.mode == 'answer'):
            ...
            answer_choices = item['aug_rationales']
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.h5fn, 'r') as h5:
            grp_items = {k: np.asarray(v, dtype=np.float16) for k,v in h5[str(index)].items()}
        # grp_items = {k: np.array(v, dtype=np.float16) for k,v in self.h5[str(index)].items()}
        ...
        if self.mode == 'answer':
            ...
            instance_dict['label'] = LabelField(item['aug_answer_rationale_indexes'][item['answer_label']], skip_indexing=True)
