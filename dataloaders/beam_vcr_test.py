...
import config
from config import VCR_IMAGES_DIR,  VCR_DIR
...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da', aug_flag=True):
        ...
        # # TODO: load COCO
        with open(os.path.join(VCR_DIR, 'dataloaders', 'cocoontology.json'), 'r') as f:
            coco = json.load(f)
        ...
        # self.h5fn = os.path.join(config.VCR_ANNOTS_DIR, f'beam_bert_rationale_{self.split}.h5')
        self.h5fn = os.path.join(config.VCR_ANNOTS_DIR, f'aug_beam_bert_rationale_{self.split}.h5')
    ...
    def __getitem__(self, index):
        ...
        item_index = (int)(index/4);
        action_index = (int)(index%4);
        item = deepcopy(self.items[item_index])

        ###################################################################
        # Load questions and answers
        if self.mode == 'rationale':
            # item['question'] += item['answer_choices'][item['answer_label']]
            item['question'] += item['answer_choices'][action_index]
        if self.mode == 'answer':
            answer_choices = item['answer_choices']
        else:
            answer_choices = item['aug_rationales']
        ...
        with h5py.File(self.h5fn, 'r') as h5:
            grp_items = {k: np.array(v, dtype=np.float16) for k,v in h5[str(item_index)].items()}
        ...
        if 'endingonly' not in self.embs_to_load:
            questions_tokenized, question_tags = zip(*[_fix_tokenization(
                item['question'],
                grp_items[f'ctx_{key}{action_index*16+i}'],
                old_det_to_new_ind,
                item['objects'],
        ...
        answers_tokenized, answer_tags = zip(*[_fix_tokenization(
            answer,
            grp_items[f'answer_{key}{action_index*16+i}'],
            old_det_to_new_ind,
            item['objects'],
            token_indexers=self.token_indexers,
            pad_ind=0 if self.add_image_as_a_box else -1
